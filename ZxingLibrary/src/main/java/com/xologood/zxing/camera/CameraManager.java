...
package com.xologood.zxing.camera;
...
public final class CameraManager {
	...
	public static int scanframe = 1;//1 二维码扫描   2条形码扫描
...
//	public synchronized void setFramingRect(int num) {
//		if (framingRect != null) {
//			framingRect = null;
//		}
//		scanframe = num;
...
//						MIN_FRAME_WIDTH, MAX_FRAME_WIDTH);   
				width = screenResolution.x * 5 / 8;  //扫描框的宽度
				height = width;  //扫描框的高度
			}else if(scanframe == 2){
				width = screenResolution.x * 7 / 8; //扫描框的宽度
				height = screenResolution.y * 1 / 8; //扫描框的高度
			}
					
			int leftOffset = (screenResolution.x - width) / 2;//扫描框的靠左的宽度
			int topOffset = (screenResolution.y - height) / 3;//扫描框的靠上的宽度
			
			//扫描框的靠左的宽度，扫描框的靠上的宽度，作为坐标原点 开始描绘扫描框 
